...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(new URL('/oauth-callback?error=google_oauth_failed', request.url));
    }
    ...
    // If we do get a code, save the credentials to Cal.com
    await saveCalendarCredentials('google', code, state || '');

    console.log('Google OAuth credentials saved successfully to Cal.com');
